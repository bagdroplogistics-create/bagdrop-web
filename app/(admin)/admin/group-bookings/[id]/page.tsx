...
interface Bag {
  id: string; guest_id: string | null; bag_number: string; bag_label: string | null; status: string
  ...
  // Airline info — separate from BagDrop's own Bag ID/QR (bag_number/bag_label).
  airline_name: string | null; flight_number: string | null; pnr: string | null
  passenger_name: string | null; airline_tag_number: string | null; airline_barcode: string | null
}

// BagDrop Bag Tag System's canonical 8-stage vocabulary (lib/bag-tags.ts)
// — see supabase/migrations/20260905_bagdrop_bag_tags.sql. Replaces the
// old placeholder statuses this page shipped with (pending/received/
// tagged/etc.), which were never wired to a real dashboard.
const STATUS_META: Record<string, { label: string; color: string; bg: string }> = {
  tag_generated:      { label: 'Tag Generated',      color: '#6b7280', bg: '#f3f4f6' },
  tag_printed:        { label: 'Tag Printed',        color: '#7c3aed', bg: '#ede9fe' },
  pickup_pending:     { label: 'Pickup Pending',      color: '#b45309', bg: '#fef3c7' },
  picked_up:          { label: 'Picked Up',          color: '#0891b2', bg: '#cffafe' },
  airport_handover:   { label: 'Airport Handover',    color: '#4f46e5', bg: '#eef2ff' },
  in_transit:         { label: 'In Transit',          color: '#0369a1', bg: '#e0f2fe' },
  delivered:          { label: 'Delivered',           color: '#16a34a', bg: '#dcfce7' },
  delivery_exception: { label: 'Delivery Failed / Exception', color: '#dc2626', bg: '#fee2e2' },
}
...
export default function GroupBookingDetailPage() {
  ...
  const BAG_FORM_DEFAULTS = {
    guest_id: '', pickup_location: '', delivery_location: '', hotel_name: '', room_number: '', remarks: '', status: 'tag_generated',
    // Airline info — explicitly separate from BagDrop's own Bag ID/QR,
    // staff-entered only, never generated by this system.
    airline_name: '', flight_number: '', pnr: '', passenger_name: '', airline_tag_number: '', airline_barcode: '',
  }
  const [bagForm, setBagForm] = useState(BAG_FORM_DEFAULTS)
  ...
  function openBagModal(bag: Bag | null) {
    ...
    setBagForm(bag ? {
      ...
      airline_name: bag.airline_name ?? '', flight_number: bag.flight_number ?? '', pnr: bag.pnr ?? '',
      passenger_name: bag.passenger_name ?? '', airline_tag_number: bag.airline_tag_number ?? '', airline_barcode: bag.airline_barcode ?? '',
    } : { ...BAG_FORM_DEFAULTS, pickup_location: group?.pickup_address ?? '', delivery_location: group?.delivery_address ?? '', hotel_name: group?.hotel_name ?? '' })
  ...
  async function deleteBag(bag: Bag) {
    if (!confirm(`Remove bag ${bag.bag_label ?? bag.bag_number}? This ID will never be reused.`)) return
  ...
  const bagStats = {
    total:            bags.length,
    pendingPickup:    bags.filter(b => ['tag_generated', 'tag_printed', 'pickup_pending'].includes(b.status)).length,
    pickedUp:         bags.filter(b => b.status === 'picked_up').length,
    airportHandover:  bags.filter(b => b.status === 'airport_handover').length,
    inTransit:        bags.filter(b => b.status === 'in_transit').length,
    delivered:        bags.filter(b => b.status === 'delivered').length,
    pendingDelivery:  bags.filter(b => ['picked_up', 'airport_handover', 'in_transit'].includes(b.status)).length,
  }
  const filteredBags = bags.filter(b => {
    ...
    return b.bag_number.toLowerCase().includes(q)
      || (b.bag_label ?? '').toLowerCase().includes(q)
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6">
        ...
        {/* Bag Tag operational dashboard — founder spec 2026-09-05:
            "For group bookings show: Total Bags, Picked Up, Pending
            Pickup, Airport Handover, In Transit, Delivered, Pending
            Delivery." Pending Pickup rolls up every pre-pickup stage
            (tag_generated/tag_printed/pickup_pending); Pending Delivery
            rolls up every post-pickup, pre-delivery stage (picked_up/
            airport_handover/in_transit) — Picked Up/Airport Handover/
            In Transit are also broken out individually right after it. */}
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
          <StatChip label="Total Bags" value={bagStats.total} />
          <StatChip label="Pending Pickup" value={bagStats.pendingPickup} color="#b45309" />
          <StatChip label="Picked Up" value={bagStats.pickedUp} color="#0891b2" />
          <StatChip label="Airport Handover" value={bagStats.airportHandover} color="#4f46e5" />
          <StatChip label="In Transit" value={bagStats.inTransit} color="#0369a1" />
          <StatChip label="Delivered" value={bagStats.delivered} color="#16a34a" />
          <StatChip label="Pending Delivery" value={bagStats.pendingDelivery} color="#ea580c" />
        </div>
                  ...
                  {filteredBags.map(b => {
                    ...
                    return (
                      <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-2 font-mono text-xs font-bold text-gray-700">{b.bag_label ?? <span className="text-gray-400">Not tagged yet</span>}</td>
      ...
      {bagModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">{bagModal.bag ? `Edit Bag ${bagModal.bag.bag_label ?? bagModal.bag.bag_number}` : 'Add Bag'}</h2>
              ...
              {bagModal.bag && (
                <div className="mt-2 rounded-lg border border-gray-100 bg-gray-50 p-3">
                  <p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-gray-400">
                    Airline Information <span className="font-normal normal-case text-gray-400">— separate from BagDrop&apos;s own Bag ID/QR, never generated by BagDrop</span>
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className={labelCls}>Airline</label><input className={inputCls} value={bagForm.airline_name} onChange={e => setBagForm(f => ({ ...f, airline_name: e.target.value }))} /></div>
                    <div><label className={labelCls}>Flight Number</label><input className={inputCls} value={bagForm.flight_number} onChange={e => setBagForm(f => ({ ...f, flight_number: e.target.value }))} /></div>
                    <div><label className={labelCls}>PNR</label><input className={inputCls} value={bagForm.pnr} onChange={e => setBagForm(f => ({ ...f, pnr: e.target.value }))} /></div>
                    <div><label className={labelCls}>Passenger Name</label><input className={inputCls} value={bagForm.passenger_name} onChange={e => setBagForm(f => ({ ...f, passenger_name: e.target.value }))} /></div>
                    <div><label className={labelCls}>Airline Bag Tag / LPN</label><input className={inputCls} value={bagForm.airline_tag_number} onChange={e => setBagForm(f => ({ ...f, airline_tag_number: e.target.value }))} /></div>
                    <div><label className={labelCls}>Airline Barcode</label><input className={inputCls} value={bagForm.airline_barcode} onChange={e => setBagForm(f => ({ ...f, airline_barcode: e.target.value }))} /></div>
                  </div>
                </div>
              )}
            </div>

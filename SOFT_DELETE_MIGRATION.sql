...
--   The admin Leads tab's Delete button, and the Dashboard Analytics KPIs,
--   both already rely on this deleted_at column existing. Without it,
--   clicking Delete on a lead (e.g. the Moni Patel test entry) appears to
--   succeed and cancels the linked booking, but the lead itself is never
--   actually marked deleted — so it keeps counting toward Total Inquiries
--   and every other KPI. Run this once, then delete that lead again.
-- ─────────────────────────────────────────────────────────────────────────────

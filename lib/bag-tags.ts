...
// Moved to lib/bag-tag-display.ts (2026-09-07, same day — production
// incident fix) so they stay importable from the 'use client'
// components/admin/BagTagPrintCard.tsx without also dragging this file's
// top-level `import { supabaseAdmin } from './supabase'` into the browser
// bundle (that module builds its client with the server-only service-role
// key at import time, which throws "supabaseKey is required" the instant
// it runs outside a server context). Re-exported here so the existing
// server-side import in lib/bag-tags-pdf.tsx keeps working unchanged —
// see lib/bag-tag-display.ts's own module comment for the full story.
export { cityCode, barcodeStripes, type BarcodeStripe } from './bag-tag-display'

    ...
    >
      ...
      <div className="space-y-1.5">
        ...
        <input
          ...
          value={state.date}
          onChange={e => {
            const updates: Partial<typeof state> = { date: e.target.value }
            // Clear delivery date if it's now before the new pickup date
            if (state.deliveryDate && state.deliveryDate < e.target.value) {
      ...
      {/* ── Delivery Date ── */}
      <div className="space-y-1.5">
        <label htmlFor="delivery-date" className="block text-base font-medium text-text-primary">
        ...
        <input
          id="delivery-date"
          type="date"
          min={state.date || minDate()}
          value={state.deliveryDate}
          onChange={e => onChange({ deliveryDate: e.target.value })}
          className="input-base"
        />
        <p className="text-xs text-text-muted">When do you need your bags delivered? We'll plan accordingly.</p>
      </div>

      {/* ── Pickup Time ── */}
      <div className="space-y-1.5">
        <label htmlFor="pickup-time" className="flex items-center gap-1.5 text-base font-medium text-text-primary">
        ...
        <p className="text-xs text-text-muted">All timings in IST (Indian Standard Time).</p>
      </div>

...
export function StepReview({ state, onChange, onBack, onBook }: StepReviewProps) {
  ...
  const formattedDeliveryDate = state.deliveryDate
    ? new Date(state.deliveryDate + 'T00:00:00').toLocaleDateString('en-IN', {
        weekday: 'short', day: 'numeric', month: 'short', year: 'numeric',
      })
    : ''
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-border bg-white p-5 space-y-3 text-sm">
            ...
            <SummaryRow icon={Calendar} label="Pickup date"   value={formattedDate || '—'} />
            {formattedDeliveryDate && (
              <SummaryRow icon={Calendar} label="Delivery date" value={formattedDeliveryDate} />
            )}

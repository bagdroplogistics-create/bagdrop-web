...
export interface ServicePageProps {
  ...
  ctaBody:     string
  bookHref?:   string
  /** URL slug, e.g. "excess-baggage". Enables breadcrumb nav + schema. */
  slug?:       string
}
...
export function ServicePage({
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      {/* FAQPage JSON-LD — enables Google rich results (expandable Q&A in search) */}
      {faqs.length > 0 && (
        <Script
          id={`schema-faq-${slug ?? 'page'}`}
          type="application/ld+json"
          strategy="beforeInteractive"
      ...
      <nav aria-label="Breadcrumb" className="bg-white border-b border-border">
        <div className="section-container py-3">
          <ol className="flex items-center gap-1.5 text-sm text-text-muted flex-wrap">
            ...
            <li aria-hidden="true"><ChevronRight className="h-3.5 w-3.5 text-text-muted/40" /></li>
            <li className="text-text-primary font-medium" aria-current="page">{badge}</li>
          </ol>
        </div>
      </nav>
